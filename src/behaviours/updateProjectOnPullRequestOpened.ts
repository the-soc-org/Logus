...
export default class UpdateProjectOnPullRequestOpened implements Behaviour {
  ...
  register(agent: Probot): void {
    agent.on("pull_request.opened", async (context) =>
      this.updateDateAction(agent, context),
    );
  }
   ...
   */
  private async updateDateAction(
    agent: Probot,
    ...
    agent.log.info(
      `Pull request ${context.payload.number} in the repo ${repoId} opened at ${createdAt}.`,
    );
    ...
    await fieldUpdater.updateDate(
      (s) => s.openPullRequestDateProjectFieldName,
      createdAt,
    );
  }
}
